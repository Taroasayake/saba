...
//#include "libavutil/avutil.h"             は C++対応用に改造してある
...
#define FF_VIDEO_OUT_EVENT      (SDL_USEREVENT + 3)
static int eventdisableflag = 0;
...
void RenewVideoTex()
{
    SDL_LockMutex(api_mutex);

    // いままで提供していたテクスチャーがあれば削除する
...
{
    // 古いTextureは削除する?
    // 更新はRead Threadのタイミングでやる必要あり

    SDL_LockMutex(api_mutex);
    GLuint returnID = 0;
    returnID = m_dummyImageTex2;
    SDL_UnlockMutex(api_mutex);
    return returnID;
}
...
static void video_image_display(VideoState *is)
{
    ...
    if (!vp->uploaded) {
        ...
        is->img_convert_ctx = sws_getCachedContext(is->img_convert_ctx,
            vp->frame->width, vp->frame->height, (enum AVPixelFormat)vp->frame->format, 
        ...
        if (is->img_convert_ctx != NULL) 
        {
            ...
            int num_bytes = av_image_get_buffer_size(AV_PIX_FMT_RGB24, vp->frame->width, vp->frame->height, 1);
            ...
            //Convert YUV->RGB
            sws_scale(is->img_convert_ctx, vp->frame->data
...

            {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, vp->frame->width, vp->frame->height, 0, GL_RGB, GL_UNSIGNED_BYTE, dst_data[0]);
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            strream_bitrate = is->ic->bit_rate;
            //strcpy(video_format, (char*)av_x_if_null(av_get_pix_fmt_name(frame->format), "none"));
            //av_log(NULL, AV_LOG_DEBUG,
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    if (forced_codec_name)
    {
        ...
        //strcpy(video_format, video_codec_name);
    }
    if (!codec) {
...
static int read_thread(void *arg)
{
    ...
    format_opts = NULL;     // 起動時のオプションが効かなくなる可能性あり
...
//      of the seek_pos/seek_rel variables
        ...
        if (!is->paused &&
            ...
            {
                {
                    if (eventdisableflag == 0)
                    {
                        SDL_Event event;
                        event.type = FF_VIDEO_OUT_EVENT;
                        event.user.data1 = is;
                        SDL_PushEvent(&event);
                        eventdisableflag = 1;
                    }
...
//            SDL_ShowCursor(0);
//            cursor_hidden = 1;
//            //av_log(NULL, AV_LOG_INFO, "Cursor Off\n");
//        }
...
double ffplay_getpos()
{
    ...
    if (is != 0)
    {
        pos = get_master_clock(is);                    // 秒のdouble
    }
    else
    {
        pos = 0;
...
int64_t ffplay_getstreamsecond()
{
    ...
    if (is != 0)
    {
        ...
        if (cur_stream->ic != 0)
        {
            len = cur_stream->ic->duration / 1000LL;       // ミリ秒の整数
        }
        else
        {
            len = 0;
...
void ffplay_seekpos(double pos)
{
    ...
    {
        VideoState* cur_stream = is;
        double oldpos = get_master_clock(cur_stream);
        // 秒数だから そのまま使って良い?
        if (cur_stream->ic->start_time != AV_NOPTS_VALUE && pos < cur_stream->ic->start_time / (double)AV_TIME_BASE)
            pos = cur_stream->ic->start_time / (double)AV_TIME_BASE;
...
int ffplay_event(SDL_Event event)
{
    ...
    if (cur_stream != 0)
    {
        // いったんcur_streamがあるときだけイベントを処理するようにする
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_AC_BACK:
            {
                SDL_Keymod ks = SDL_GetModState();
                if ((ks & KMOD_LSHIFT) || (ks & KMOD_RSHIFT))
                {
                    incr = -(double)cur_stream->ic->duration / 1000. / 1000. / 5;
                }
                else
                {
                    incr = seek_interval ? -seek_interval : -10.0;
            ...
            //    //incr = 60.0;
            //    //goto do_seek;
            //    returnstatus = 3;       // 前のビデオ
            //    break;
            //case SDLK_DOWN:
            //    //incr = -60.0;
            //    returnstatus = 2;       // Video Endと同じで次のVideoに行く
            //    break;
            do_seek:
                if (seek_by_bytes) {
                    pos = -1;
                    ...
                    if ((pos < 0.1) && (incr < 0))
                    {
                        //returnstatus = 3;     // 前の動画に遷移する

                        // 同じ動画の最後尾に移動する
                        pos = cur_stream->ic->duration - 5;
                    }
                    else
                    {
                        pos += incr;
        ...
        case SDL_MOUSEMOTION:
            if (cursor_hidden) {
                SDL_ShowCursor(1);
                cursor_hidden = 0;
                //av_log(NULL, AV_LOG_INFO, "Cursor On\n");
            }
            cursor_last_shown = av_gettime_relative();
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                if (event.button.button != SDL_BUTTON_RIGHT)
        ...
        case FF_VIDEO_OUT_EVENT:
            // Video Stream out
            av_log(NULL, AV_LOG_INFO, "Video End\n");
            //ffplay_seekpos(0);
            returnstatus = 1;       // video is ended
            break;
...
int changeVideo(const char* filename)
{
    ...
    if (filename != 0)
    {
        ...
        {
            av_log(NULL, AV_LOG_INFO, "Success to initialize VideoState!\n");

            volatile int x = 0;
            while (fileopenstatus == 0)
            {
                ++x;
            }
